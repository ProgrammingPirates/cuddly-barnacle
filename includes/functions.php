...
class iN_UPDATES {
	...
	*/
	public function iN_CheckUserName($username) {
		...
		$query = mysqli_query($this->db, "SELECT * FROM i_users WHERE i_username = '$username'") or die(mysqli_error($this->db));
	...
	*/
	public function iN_Languages() {
        $status = '1'; // ENUM veya VARCHAR ise string olarak bağlanmalı
        $stmt = $this->db->prepare("SELECT * FROM i_langs WHERE lang_status = ?");
	...
	public function iN_SelectedMenuIcon($icon_id) {
        $stmt = $this->db->prepare("SELECT icon_id, icon_code, icon_status FROM i_svg_icons WHERE icon_status = ? AND icon_id = ?");
        $status = 1;
        $stmt->bind_param("si", $status, $icon_id); // i = integer
        $stmt->execute();
        $result = $stmt->get_result();
	...
	public function iN_UserFullName($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		$query = mysqli_query($this->db, "SELECT i_user_fullname, i_username FROM `i_users` WHERE iuid='$userID' AND uStatus IN('1','3')") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		if (isset($data['i_user_fullname'])) {
		    $myUsername = $data['i_username'];
			$fullName = isset($data['i_user_fullname']) ? $data['i_user_fullname'] : $myUsername;
	...
	public function iN_GetPCo() {
		$query = mysqli_query($this->db, "SELECT * FROM  i_configurations WHERE configuration_id = '1'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($data['mycd']) ? $data['mycd'] : FALSE;
	}
	...
	public function iN_GetNewNotificationSum($uid) {
		...
		if ($this->iN_CheckUserExist($uid) == 1) {
			...
			return isset($row['newNotification']) ? $row['newNotification'] : '0';
		} else {
			return false;
		}
	}
	...
	public function iN_InsertNewPost($uid, $postText, $urlSlug, $postFiles, $postWhoCanSee, $hashTags, $pointAmount,$autoApprovePostStatus) {
		...
		$userIP = $_SERVER['REMOTE_ADDR'];
		$postStatus = '0';
		if (!empty($postFiles) && trim($postFiles) === '') {
			$postStatus = '1';
		}
		if ($postWhoCanSee == '4' && $autoApprovePostStatus == 'no') {
			$postStatus = '2';
		}else{
			$postStatus = '1';
		}
		...
		$newPostID = isset($result['post_id']) ? $result['post_id'] : NULL;
		if($newPostID){
	...
	public function iN_GetPostComments($postID, $second) {
		...
		$DataComments = mysqli_query($this->db, "SELECT C.com_id,C.comment_uid_fk,C.comment_post_id_fk,C.comment,C.comment_time,C.comment_file,C.sticker_url, C.gif_url,U.iuid,U.i_username,U.i_user_fullname,U.user_avatar,U.user_gender,U.last_login_time,U.user_verified_status,U.user_frame
        ...
        if ($this->iN_CheckPostIDExist($postID) == '1') {
            $getData = mysqli_query($this->db, "
                SELECT DISTINCT
                    ...
                    P.post_want_status, P.post_wanted_credit, P.post_status,
            ...
            return mysqli_fetch_array($getData, MYSQLI_ASSOC);
        } else {
            return false;
        }
    }
	...
	public function iN_TotalImagePosts($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		$countImagePosts = mysqli_query($this->db, "SELECT COUNT(*) AS totalImagePost FROM i_user_uploads WHERE iuid_fk = '$userID' AND upload_status = '1' AND upload_type = 'wall' AND uploaded_file_ext IN('gif','GIF','jpg','jpeg','JPEG','JPG','PNG','png')") or die(mysqli_error($this->db));
	...
	public function iN_TotalVideoPosts($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		$countVideoPosts = mysqli_query($this->db, "SELECT COUNT(*) AS totalVideoPost FROM i_user_uploads WHERE iuid_fk = '$userID' AND upload_status = '1' AND upload_type = 'wall' AND uploaded_file_ext IN('mp4','MP4')") or die(mysqli_error($this->db));
	...
	public function iN_TotalAudioPosts($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		$countVideoPosts = mysqli_query($this->db, "SELECT COUNT(*) AS totalAudioPost FROM i_user_uploads WHERE iuid_fk = '$userID' AND upload_status = '1' AND upload_type = 'wall' AND uploaded_file_ext IN('mp3','MP3')") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($countVideoPosts, MYSQLI_ASSOC);
	...
	public function iN_UnSubscriberUser($userID, $uID,$unSubscribeStyle) {
		...
		if ($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckUserExist($uID) == 1) {
			...
			if($unSubscribeStyle == 'no'){
			    mysqli_query($this->db, "UPDATE i_friends SET fr_status = 'flwr' WHERE fr_one = '$userID' AND fr_two='$uID'") or die(mysqli_error($this->db));
				return true;
			}
	...
	public function iN_AllUserProfilePosts($uid, $lastPostID, $showingPost) {
		...
		FROM i_friends F
			...
			INNER JOIN i_users U
			ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3') AND F.fr_status IN('me', 'flwr', 'subscriber')
		WHERE P.post_owner_id='$uid' AND P.post_pined = '0' $morePost
		ORDER BY P.post_id
		DESC LIMIT $showingPosts ") or die(mysqli_error($this->db));
	...
	public function iN_PaymentsSubscriptionsList($userID, $paginationLimit, $page) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			...
			WHERE S.status IN('active','inactive') AND in_status IN('1','0') AND finished IN('0','1') AND S.subscribed_iuid_fk = '$userID' ORDER BY S.subscription_id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function CheckFramePlanExist($planID) {
		$planID = mysqli_real_escape_string($this->db, $planID);
		$query = mysqli_query($this->db, "SELECT f_id FROM i_frames WHERE f_id = '$planID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($query) == 1) {
			return true;
		} else {return false;}
	}
	...
	public function iN_LatestPaymentPoint($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckUserExist($userID) == '1') {
			$query = mysqli_query($this->db, "SELECT * FROM i_user_payments WHERE payer_iuid_fk = '$userID' AND payment_type IN('point','product') AND payment_status = 'pending' AND payment_time >= UNIX_TIMESTAMP(NOW() - INTERVAL 1 MINUTE)") or die(mysqli_error($this->db));
            $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $data;
		} else {
			return false;
...
public function iN_ChatUserList($userID, $limit) {
    ...
    if ($this->iN_CheckUserExist($userID) == '1') {
        $query = mysqli_query($this->db, "
            SELECT DISTINCT
                ...
                U.i_user_fullname,
                U.user_avatar,
                U.online_offline_status,
                U.last_login_time,
                U.user_gender,
                U.user_verified_status
            FROM
                i_chat_users C FORCE INDEX (ixUserChat)
            INNER JOIN
                i_users U FORCE INDEX (ixForceUser)
                ON C.user_one = U.iuid
            WHERE
                U.uStatus IN ('1','3') AND
                (C.user_two = '$userID' OR C.user_one = '$userID')
            ORDER BY
	...
	public function iN_GetChatMessages($userID, $chatID, $lastMessageID, $messageLimit) {
		...
		if ($this->iN_CheckUserExist($userID) == '1' && $this->iN_CheckChatIDExist($chatID) == '1') {
			...
			$query = mysqli_query($this->db, "SELECT * FROM (SELECT DISTINCT
				M.con_id, M.chat_id_fk, M.user_one, M.user_two,M.private_price, M.private_status,M.gifMoney,M.message,M.seen_status,M.file,M.sticker_url,M.gifurl, M.time,C.chat_id, U.iuid, U.uStatus, U.i_username, U.i_user_fullname, U.user_gender
	...
	public function iN_GetUserNewMessage($uid, $chatID, $user, $lastMessageID) {
		...
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$query = mysqli_query($this->db, "SELECT DISTINCT
				M.con_id, M.chat_id_fk, M.user_one, M.user_two,M.gifMoney,M.private_price, M.private_status,M.message,M.file,M.seen_status,M.sticker_url,M.gifurl, M.time,C.chat_id, U.iuid, U.uStatus, U.i_username, U.i_user_fullname, U.user_gender
				FROM i_chat_users C FORCE INDEX(ixUserChat)
				INNER JOIN i_chat_conversations M FORCE INDEX(ixChat)
				ON C.chat_id = M.chat_id_fk
				INNER JOIN i_users U FORCE INDEX(ixForceUser)
	...
	public function iN_InsertNewMessage($userID, $chatID, $message, $stickerURL, $gif, $fileIDs, $mMoney) {
		...
		if ($this->iN_CheckUserExist($userID) == '1' && $this->iN_CheckChatIDExist($chatID) == '1') {
			...
			mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
			mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
			$query = mysqli_query($this->db, "INSERT INTO i_chat_conversations(chat_id_fk,user_one, user_two,message,sticker_url,gifurl,file, time, private_status, private_price)VALUES('$chatID','$userID','$toID', '$message','$stickerURL', '$gif','$fileIDs', '$time', 'closed','$mMoney')") or die(mysqli_error($this->db));
			mysqli_query($this->db,"UPDATE i_chat_users SET last_message_time = '$time' WHERE chat_id = '$chatID'") or die(mysqli_error($this->db));
			if ($query) {
				$lastMessageData = mysqli_query($this->db, "SELECT * FROM (SELECT DISTINCT
				M.con_id, M.chat_id_fk, M.user_one, M.user_two,M.gifMoney,M.private_price, M.private_status,M.message,M.file,M.sticker_url,M.gifurl,M.seen_status, M.time,C.chat_id, U.iuid, U.uStatus, U.i_username, U.i_user_fullname, U.user_gender
				FROM i_chat_users C FORCE INDEX(ixUserChat)
				INNER JOIN i_chat_conversations M FORCE INDEX(ixChat)
				ON C.chat_id = M.chat_id_fk
				INNER JOIN i_users U FORCE INDEX(ixForceUser)
	...
	public function iN_GetUploadedMessageFileDetails($imageID) {
        ...
        $stmt = $this->db->prepare("SELECT * FROM i_user_conversation_uploads WHERE upload_id = ?");
	...
	public function iN_PopularUsersFromLastWeek() {
        $db = $this->db;
        $sql = "SELECT DISTINCT P.post_owner_id, U.iuid, U.i_username, U.i_user_fullname, U.user_verified_status, U.user_gender, U.certification_status, U.validation_status, U.condition_status, U.fees_status, count(P.post_owner_id) as cnt
            FROM i_posts P FORCE INDEX(ixForcePostOwner)
            INNER JOIN i_users U FORCE INDEX(ixForceUser)
            ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3')
            WHERE WEEK(FROM_UNIXTIME(P.post_created_time)) = WEEK(NOW()) - 1 AND U.certification_status = '2' AND U.validation_status = '2' AND U.condition_status = '2' AND U.fees_status = '2'
	...
	public function iN_PopularUsersFromLastWeekInExplorePage() {
		$query = mysqli_query($this->db, "SELECT DISTINCT
		P.post_owner_id,P.post_owner_id, U.iuid,U.user_frame, U.i_username,U.i_user_fullname,U.user_verified_status, U.user_gender,U.certification_status,U.validation_status,U.condition_status,U.fees_status, count(P.post_owner_id) as cnt
		FROM i_posts P FORCE INDEX(ixForcePostOwner)
		INNER JOIN i_users U FORCE INDEX(ixForceUser)
		ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3')
		WHERE U.user_verified_status = '1' AND WEEK(FROM_UNIXTIME(P.post_created_time)) = WEEK(NOW()) - 1 AND U.certification_status = '2' AND U.validation_status = '2' AND U.condition_status = '2' AND U.fees_status = '2' GROUP BY P.post_owner_id ORDER BY cnt DESC LIMIT 30
		") or die(mysqli_error($this->db));
	...
	public function iN_GetSearchResult($key, $showingNumberOfPost, $Users) {
        ...
        if ($Users == 'yes') {
            $result = mysqli_query($this->db, "SELECT * FROM i_users WHERE (i_username LIKE '$key' OR i_user_fullname LIKE '$key' OR i_username LIKE '$key' OR i_user_fullname LIKE '$key') AND uStatus IN('1','3') AND certification_status = '2' AND validation_status = '2' AND condition_status = '2' AND fees_status = '2' AND payout_status = '2' ORDER BY iuid LIMIT $showingNumberOfPost") or die(mysqli_error($this->db));
        } else {
            $result = mysqli_query($this->db, "SELECT * FROM i_users WHERE (i_username LIKE '$key' OR i_user_fullname LIKE '$key' OR i_username LIKE '$key' OR i_user_fullname LIKE '$key') AND uStatus IN('1','3') ORDER BY iuid LIMIT $showingNumberOfPost") or die(mysqli_error($this->db));
        }
	...
	public function iN_UpdateLimitValues($userID,$friendActivityShowTimeLimit, $friendActivityShowLimit, $TrendPostShowLimit,$sugProductShowLimit,$sugUserShowLimit,$oneSignalStatus,$oneSignalApiKey, $oneSignalRestApiKey, $reCaptchaStatus, $reCaptchaSiteKey, $reCaptchaSecretKey, $postCreateLimit,$blockCountryStatus, $fileLimit, $lengthLimit, $postShowLimit, $paginatonLimit, $approvalFileExtension, $availableUploadFileExtensions, $ffmpegPath, $unavailableUserNames, $messageScrollNumber, $adsShowLimit) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET activity_show_time_limit = '$friendActivityShowTimeLimit' ,activity_show_limit = '$friendActivityShowLimit' , howManyDaysTrend = '$TrendPostShowLimit' , showingNumberOfProduct = '$sugProductShowLimit', showingNumberOfSuggestedUser = '$sugUserShowLimit' ,g_recaptcha_status = '$reCaptchaStatus', g_recaptcha_site_key = '$reCaptchaSiteKey', g_recaptcha_secret_key = '$reCaptchaSecretKey', normal_user_can_post = '$postCreateLimit', available_verification_file_extensions = '$approvalFileExtension', available_file_extensions = '$availableUploadFileExtensions', available_file_size = '$fileLimit', available_length = '$lengthLimit', load_more_limit = '$postShowLimit', pagination_limit = '$paginatonLimit' , ffmpeg_path = '$ffmpegPath' , disallowed_usernames = '$unavailableUserNames',one_signal_api = '$oneSignalApiKey', one_signal_rest_api = '$oneSignalRestApiKey', one_signal_status = '$oneSignalStatus', load_more_message_limit = '$messageScrollNumber' , showingNumberOfAds = '$adsShowLimit' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateWasabiDetails($userID, $updateWasRegion, $updateWasBucket, $updateWasKey, $updateWasSecretKey, $updateWasStatus) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET ocean_status = '0' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "UPDATE i_configurations SET was_region = '$updateWasRegion', was_bucket = '$updateWasBucket' , was_key = '$updateWasKey' , was_secret_key = '$updateWasSecretKey' , was_status = '$updateWasStatus' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
public function iN_aWaitingApprovalOrApprovedPostsList($userID, $paginationLimit, $page) {
    ...
    if ($this->iN_CheckIsAdmin($userID) == 1) {
        $query = mysqli_prepare($this->db, "SELECT P.*, U.iuid, U.i_username, U.i_user_fullname, U.user_avatar, U.user_gender, U.last_login_time, U.user_verified_status
            FROM i_posts AS P
            ...
            LIMIT ?, ?
            ");
        mysqli_stmt_bind_param($query, "ii", $start_from, $paginationLimit);
	...
	public function iN_UpdateFramePlanStatus($userID, $mod, $planID) {
		...
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckFramePlanExist($planID)) {
			mysqli_query($this->db, "UPDATE i_frames SET f_frame_status = '$mod' WHERE f_id = '$planID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_AllVerficationRequestList($userID, $paginationLimit, $page) {
		...
		$start_from = ($page - 1) * $paginationLimit;
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT * FROM i_verification_requests WHERE request_status IN('0','1') ORDER BY request_id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_UpdatePayoutStatus($userID, $id) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$time = time();
			mysqli_query($this->db, "UPDATE i_user_payouts SET status = 'payed', payout_time = '$time' WHERE payout_id = '$id'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_ShowAds($numberShow) {
		$query = mysqli_query($this->db, "SELECT * FROM i_advertisements WHERE ads_status = '1' ORDER BY RAND() LIMIT $numberShow") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_YourPaymentsList($userID, $paginationLimit, $page) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			...
			WHERE P.payment_status = 'ok' AND P.payer_iuid_fk = '$userID' AND P.payment_type IN('post','live_stream','tips','live_gift','videoCall','boostPost','frame') ORDER BY P.payment_id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_CheckLiveIDExistAndOwner($userID, $liveID) {
		...
		if (mysqli_num_rows($checkPostisExist) > 0) {
			return true;
		} else {return false;}
	}
    ...
    public function iN_SuggestionCreatorsList($uid, $numberShow) {
		...
		SELECT iu.*
            FROM i_users iu
            LEFT JOIN i_friends ifr ON iu.iuid = ifr.fr_two AND ifr.fr_one = '$uid'
        WHERE iu.uStatus = '3'
            AND ifr.fr_two IS NULL
        ORDER BY RAND()
        LIMIT $numberShow") or die(mysqli_error($this->db));
		$data = array();
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			$data[] = $row;
		}
    ...
    public function iN_SuggestionCreatorsListOut($numberShow) {
		$query = mysqli_query($this->db, "SELECT * FROM i_users WHERE uStatus IN('3') AND certification_status = '2' AND validation_status = '2' AND condition_status = '2' AND fees_status = '2' ORDER BY rand() LIMIT $numberShow") or die(mysqli_error($this->db));
		$data = array();
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			$data[] = $row;
		}
	...
	public function iN_CheckCountryBlocked($userID, $value) {
		...
		if ($this->iN_CheckUserExist($userID) == '1' && !empty($value)) {
			$queryCheck = mysqli_query($this->db, "
				SELECT 1 FROM i_user_blocked_countries
				WHERE b_iuid_fk = '$userID' AND b_country = '$value'
				LIMIT 1
			");

			return (mysqli_num_rows($queryCheck) === 1);
		}
	...
	public function iN_InsertCountryInBlockList($userID, $blockingCountryCode) {
		...
		if ($this->iN_CheckUserExist($userID) == '1' && !empty($blockingCountryCode)) {
			$query = mysqli_query($this->db, "
				INSERT INTO i_user_blocked_countries (b_iuid_fk, b_country, b_time)
				VALUES ('$userID', '$blockingCountryCode', '$time')
			");
	...
	public function iN_RemoveCountryInBlockList($userID, $blockingCountryCode) {
		...
		if ($this->iN_CheckUserExist($userID) == '1' && !empty($blockingCountryCode)) {
			$query = mysqli_query($this->db, "
				DELETE FROM i_user_blocked_countries
				WHERE b_iuid_fk = '$userID' AND b_country = '$blockingCountryCode'
			");
			if ($query) {
				return true;
			}
	...
	public function iN_FollowerUsersListPage($userID, $paginationLimit, $page) {
        ...
        if ($this->iN_CheckUserExist($userID) == 1) {
            $query = mysqli_query($this->db, "
                ...
                WHERE F.fr_two = '$userID' AND F.fr_status = 'flwr'
                ORDER BY F.fr_id DESC
                LIMIT $start_from, $paginationLimit
            ") or die(mysqli_error($this->db));
        ...
        if ($this->iN_CheckUserExist($userID) == 1) {
            $query = "
                SELECT DISTINCT
                    ...
                    U.user_frame
                FROM i_users U
                INNER JOIN i_friends F ON F.fr_two = U.iuid
                WHERE F.fr_two = '$userID' AND F.fr_status = 'flwr' $morePost
                ORDER BY F.fr_id DESC
                LIMIT $showingPost
            ";
	...
	public function iN_FollowingUsersListProfilePage($userID, $lastPostID, $showingPost) {
        ...
        $data = array();
        if ($this->iN_CheckUserExist($userID) == 1) {
            $query = "SELECT DISTINCT F.fr_one, F.fr_two, F.fr_status,F.fr_id, U.iuid, U.i_username, U.i_user_fullname, U.user_frame
                      FROM i_users U
                      INNER JOIN i_friends F
                      ON F.fr_one = U.iuid
                      WHERE F.fr_one = '$userID' AND F.fr_status = 'flwr' $morePost
                      ORDER BY F.fr_id DESC
                      LIMIT $showingPost";
	...
	public function iN_SubscribersUsersListProfilePage($userID, $lastPostID, $showingPost) {
        ...
        if ($this->iN_CheckUserExist($userID) == 1) {
            $query = "SELECT DISTINCT F.fr_one, F.fr_two, F.fr_status,F.fr_id, U.iuid, U.i_username, U.i_user_fullname, U.user_frame
                      ...
                      INNER JOIN i_friends F
                      ON F.fr_two = U.iuid
                      WHERE F.fr_two = '$userID' AND F.fr_status = 'subscriber' $morePost
                      ORDER BY F.fr_id DESC
                      LIMIT $showingPost";
    ...
    public function iN_YourPointPaymentsHistoryList($userID, $paginationLimit, $page) {
        ...
        if ($this->iN_CheckUserExist($userID) == 1) {
            ...
            $deleteQuery = mysqli_query($this->db, "
                DELETE FROM i_user_payments
                WHERE payment_type = 'point'
                  AND payment_status = 'pending'
                  AND credit_plan_id IS NOT NULL
                  AND payment_time < '$fiveHoursAgo'
            ");

            $query = mysqli_query($this->db,"
                SELECT DISTINCT P.payment_id, P.payer_iuid_fk, P.order_key, P.payment_type,
                                P.payment_option, P.payment_time, P.payment_status, P.credit_plan_id
                FROM i_users U FORCE INDEX(ixForceUser)
                INNER JOIN i_user_payments P FORCE INDEX(ixPayment)
                    ON P.payer_iuid_fk = U.iuid AND U.uStatus IN('1','3')
                WHERE P.payer_iuid_fk = '$userID'
	...
	public function iN_InsertUserSubscriptionWithPoint($userID, $subscribedUserID, $planType, $subscriberName, $planAmount, $adminEarning, $userNetEarning, $planCurrency, $planinterval, $planIntervalCount, $subscriberEmail, $plancreated, $current_period_start, $current_period_end, $planStatus,$UpdateCurrentPoint) {
		...
		if ($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckUserExist($subscribedUserID) == 1) {
			...
			if ($query) {
				...
				mysqli_query($this->db,"UPDATE i_users SET wallet_money = wallet_money + $userNetEarning WHERE iuid = '$subscribedUserID'") or die(mysqli_error($this->db));
                mysqli_query($this->db,"UPDATE i_users SET wallet_points = wallet_points - '$planAmount' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
                mysqli_query($this->db, "UPDATE i_user_subscriptions SET plan_period_start = '$current_period_end', plan_period_end = '$current_period_start' WHERE iuid_fk = '$userID' AND subscribed_iuid_fk = '$subscribedUserID'") or die(mysqli_error($this->db));
				return true;
			} else {
				return false;
			}
	...
	public function iN_InsertNewFrameCard($userID, $giftImage, $giftPoint) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "INSERT INTO i_frames(f_file,f_price)VALUES('$giftImage','$giftPoint')") or die(mysqli_error($this->db));
			if ($query) {
				return true;
			} else {
				return false;
	...
	public function GetFramePlanDetails($planID) {
		...
		if ($this->CheckFramePlanExist($planID) == '1') {
			$query = mysqli_query($this->db, "SELECT * FROM i_frames WHERE f_id = '$planID'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $data;
		} else {return false;}
	}
	...
	public function iN_DeleteFramePlanFromData($userID, $planID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckFramePlanExist($planID) == 1) {
			mysqli_query($this->db, "DELETE FROM i_frames WHERE f_id = '$planID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_GetNewLiveMessage($liveID, $lastLiveMessageID) {
        // Ensure inputs are strings to avoid deprecation warning in PHP 8.1+
        $liveID = is_string($liveID) ? mysqli_real_escape_string($this->db, $liveID) : '';
        $lastLiveMessageID = is_string($lastLiveMessageID) ? mysqli_real_escape_string($this->db, $lastLiveMessageID) : '';

        // Check if the given live stream ID exists
        if ($this->iN_CheckLiveIDExist($liveID) == 1) {
            $lasMessageID = '';

            // If a valid last message ID is provided, filter messages after that ID
            if (!empty($lastLiveMessageID)) {
                $lasMessageID = "AND cm_id > '" . $lastLiveMessageID . "'";
            }

            // Query the new messages for the given live stream
            $query = mysqli_query($this->db, "SELECT * FROM i_live_chat WHERE cm_live_id = '$liveID' $lasMessageID ORDER BY cm_id") or die(mysqli_error($this->db));
            ...
            // Return message list if exists
            return $data;
        }

        // If live stream does not exist, return empty array
        return [];
    }
	...
	public function iN_LiveStreaminsListAllTypeBottom($lastPostID, $showingPost,$userID) {
		...
		L.live_id, L.live_name, L.live_uid_fk,L.finish_time, L.live_credit, L.live_type, U.iuid, U.i_username, U.i_user_fullname
		FROM i_live L FORCE INDEX(ix_Live)
		INNER JOIN i_users U FORCE INDEX(ixForceUser) ON L.live_uid_fk = U.iuid AND L.live_uid_fk != '$userID' AND U.uStatus IN('1','3')
		WHERE L.finish_time >= '$tree_minutes_ago' $morePost ORDER BY L.live_id DESC LIMIT $showingPost") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($getLives, MYSQLI_ASSOC)) {
			$data[] = $row;
		}
	...
	public function iN_GetHashTagsSearchResult($hashTag, $showingPosts) {
		...
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$getData = mysqli_query($this->db, "SELECT * FROM i_posts WHERE ($hashtag_query) ORDER BY post_id DESC LIMIT $showingPosts") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($getData, MYSQLI_ASSOC)) {
			// Store the result into array
			$data[] = $row;
		}
	...
	public function iN_CheckProductSeenBeforeByUserID($userID, $pID){
		$query = mysqli_query($this->db,"SELECT * FROM i_product_seen_time WHERE p_id = '$pID' AND p_s_iuid_fk = '$userID'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) == 0){
			 return true;
		 }else{
			 return false;
	 ...
	 public function iN_InsertVisitor($ip,$uProductID,$lUserID) {
		 ...
		 $time = time();
		 if($this->iN_CheckIPExist($userIP, $uProductID) == 1){
	...
	public function iN_TotalProductSell($pID) {
		$pID = mysqli_real_escape_string($this->db, $pID);
		$countProductSeen = mysqli_query($this->db, "SELECT COUNT(*) AS productSellCount FROM i_user_payments WHERE paymet_product_id = '$pID' AND payment_status = 'ok' AND payed_iuid_fk IS NOT NULL AND amount IS NOT NULL") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($countProductSeen, MYSQLI_ASSOC);
	...
	public function iN_CheckItemPurchasedBefore($userID, $productID){
		if($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckProductIDExistFromURL($productID)){
            $query = mysqli_query($this->db,"SELECT * FROM i_user_payments WHERE paymet_product_id = '$productID' AND payer_iuid_fk = '$userID' AND amount IS NOT NULL") or die(mysqli_error($this->db));
			if(mysqli_num_rows($query) > 0){
               return true;
			}else{
				return false;
	...
	/*Shop Data*/
	public function iN_ShopData($id){
	...
	public function iN_GetStoryData($id){
		$query = mysqli_query($this->db,"SELECT * FROM i_story_configuration WHERE id = '$id'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($data) ? $data : FALSE;
	}
	/*Update Post Create Mod*/
	...
	public function iN_AllUserProfilePostsByChooseAudios($uid, $lastPostID, $showingPost) {
        ...
        $getData = mysqli_query($this->db, "
            SELECT
                P.*,
                U.*,
                ANY_VALUE(A.upload_id) AS upload_id,
                ANY_VALUE(A.iuid_fk) AS iuid_fk,
                ANY_VALUE(A.uploaded_file_path) AS uploaded_file_path,
                ANY_VALUE(A.upload_tumbnail_file_path) AS upload_tumbnail_file_path,
                ANY_VALUE(A.uploaded_x_file_path) AS uploaded_x_file_path,
                ANY_VALUE(A.uploaded_file_ext) AS uploaded_file_ext,
                ANY_VALUE(A.upload_time) AS upload_time,
                ANY_VALUE(A.ip) AS ip,
                ANY_VALUE(A.upload_type) AS upload_type,
                ANY_VALUE(A.upload_status) AS upload_status
            FROM i_friends F FORCE INDEX(ixFriend)
            INNER JOIN i_posts P FORCE INDEX (ixForcePostOwner)
                ON P.post_owner_id = F.fr_two
            INNER JOIN i_users U FORCE INDEX (ixForceUser)
	...
	public function iN_AllUserProfileProductPosts($uid, $lastPostID, $showingPost){
		...
		$query = mysqli_query($this->db, "SELECT P.* , U.*
			...
			if (!empty($data)) {
                ...
                return false; // Veya başka bir varsayılan değer
            }
	}
	/*Move My Point*/
	...
	public function iN_CheckWebsiteSocialSiteIdExist($sID){
		$query = mysqli_query($this->db,"SELECT * FROM i_website_social_networks WHERE id = '$sID'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) == 1){
			return true;
		}else{
			return false;
	...
	public function iN_ShowUserSocialSites($userID){
		if($this->iN_CheckUserExist($userID) == 1){
			...
			WHERE S.uid_fk = '$userID' AND S.s_link IS NOT NULL AND S.s_link != '' ORDER BY U.id") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
			if (!empty($data)) {
	...
	public function iN_SuggestedProductWidget($showingPost) {
		...
		$query = mysqli_query($this->db, "SELECT A.*
			FROM (SELECT A.*
				...
				ORDER BY RAND()
				LIMIT $showingPost") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function iN_CheckVideoCallStatus($userID) {
		if($this->iN_CheckUserExist($userID) == 1){
			$query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE caller_uid_fk = '$userID' OR called_uid_fk = '$userID' AND DATE_FORMAT(now(), '%H-%i') = DATE_FORMAT(FROM_UNIXTIME(called_time), '%H-%i')") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
	...
	public function iN_CheckWhoCanSeePost($postID){
		$query = mysqli_query($this->db,"SELECT * FROM i_posts WHERE post_id = '$postID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($data['who_can_see']) ? $data['who_can_see'] : FALSE;
	 }
	/*Check Who Can Seee Post*/
	public function iN_GetPostOwnerIDFromPostID($postID){
		$query = mysqli_query($this->db,"SELECT * FROM i_posts WHERE post_id = '$postID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($data['post_owner_id']) ? $data['post_owner_id'] : FALSE;
	}
	public function iN_InsertNewTipMessage($userID, $chatID, $message, $sendedGiftMoney) {
		...
		if ($this->iN_CheckUserExist($userID) == '1' && $this->iN_CheckChatIDExist($chatID) == '1') {
			...
			mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
			mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
			$query = mysqli_query($this->db, "INSERT INTO i_chat_conversations(chat_id_fk,user_one, user_two,message, time ,gifMoney)VALUES('$chatID','$userID','$toID', '$message', '$time', '$sendedGiftMoney')") or die(mysqli_error($this->db));
			if ($query) {
				$lastMessageData = mysqli_query($this->db, "SELECT * FROM (SELECT DISTINCT
				M.con_id, M.chat_id_fk, M.user_one, M.user_two, M.message,M.gifMoney,M.private_price, M.private_status,M.file,M.sticker_url,M.gifurl,M.seen_status, M.time,C.chat_id, U.iuid, U.uStatus, U.i_username, U.i_user_fullname, U.user_gender
				FROM i_chat_users C FORCE INDEX(ixUserChat)
				INNER JOIN i_chat_conversations M FORCE INDEX(ixChat)
				ON C.chat_id = M.chat_id_fk
				INNER JOIN i_users U FORCE INDEX(ixForceUser)
	...
	public function iN_CheckMesageIDExistForUnLock($messageID, $conversationID) {
		$conversationID = mysqli_real_escape_string($this->db, $conversationID);
		$messageID = mysqli_real_escape_string($this->db, $messageID);
		$checkMessage = mysqli_query($this->db, "SELECT * FROM i_chat_conversations WHERE chat_id_fk = '$conversationID' AND con_id = '$messageID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkMessage) == 1) {
			return true;
		} else {
			return false;
	...
	public function iN_GetMessageDetailsByID($messageID, $chatID){
	   if($this->iN_CheckMesageIDExistForUnLock($messageID, $chatID) == 1){
		   $query = mysqli_query($this->db,"SELECT * FROM i_chat_conversations WHERE con_id = '$messageID' AND chat_id_fk = '$chatID'") or die(myqli_error($this->db));
		   $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		   return $data;
	   }else{
		   return false;
	...
	public function iN_UnLockMessage($userID, $messageID, $chatID, $adminEarning, $userEarning,$messageOwnerID, $translatePointToMoney, $adminFee, $messagePrice){
		if($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckMesageIDExistForUnLock($messageID, $chatID) == 1){
			$time = time();
			$queryInsertPayment = mysqli_query($this->db, "INSERT INTO i_user_payments(payer_iuid_fk, payed_iuid_fk, unlocked_message_id,payment_type,payment_time,payment_status, amount, fee, admin_earning, user_earning)VALUES('$userID','$messageOwnerID', '$messageID', 'unlockmessage', '$time', 'ok', '$translatePointToMoney', '$adminFee', '$adminEarning', '$userEarning')") or die(mysqli_error($this->db));
			if ($queryInsertPayment) {
				mysqli_query($this->db, "UPDATE i_users SET wallet_points = wallet_points - $messagePrice WHERE iuid = '$userID'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "UPDATE i_users SET wallet_money = (wallet_money + $userEarning) WHERE iuid = '$messageOwnerID'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "UPDATE i_chat_conversations SET private_status = 'opened' WHERE con_id = '$messageID' AND chat_id_fk = '$chatID'") or die(mysqli_error($this->db));
			}
            $query = mysqli_query($this->db,"SELECT * FROM i_chat_conversations WHERE con_id = '$messageID' AND chat_id_fk = '$chatID'") or die(myqli_error($this->db));
		    $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		    return $data;
	...
	public function iN_UpdateWhoCanSendYouAMessage($userID, $status) {
		...
		$status = mysqli_real_escape_string($this->db, $status);
		if ($this->iN_CheckUserExist($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_users SET who_can_send_message = '$status' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_CheckUserIsCreator($userID){
       $query = mysqli_query($this->db,"SELECT * FROM i_users WHERE iuid = '$userID' AND certification_status = '2' AND validation_status = '2' AND condition_status = '2' AND fees_status = '2' AND payout_status = '2'") or die(mysqli_error($this->db));
	   if(mysqli_num_rows($query) == 1){
          return true;
	   }else{
		return false;
	...
	public function iN_UpdateSarchResultStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_configurations SET show_search_result_type = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_TotalWebsiteSocialNetworks() {
		$q = mysqli_query($this->db, "SELECT COUNT(*) AS socialNetwork FROM i_website_social_networks") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($q, MYSQLI_ASSOC);
		if ($row) {
			return isset($row['socialNetwork']) ? $row['socialNetwork'] : '0';
		} else {return 0;}
	...
	public function iN_AllWebsiteSocialNetworkList($userID, $paginationLimit, $page) {
		...
		$start_from = ($page - 1) * $paginationLimit;
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT * FROM i_website_social_networks ORDER BY id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_UpdateWebsiteSocialSiteStatus($userID, $mode, $sID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckWebsiteSocialSiteIdExist($sID) == 1) {
			mysqli_query($this->db, "UPDATE i_website_social_networks SET status = '$mode' WHERE id = '$sID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_GetWbsiteSocialLinkDetails($userID, $ID){
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckWebsiteSocialSiteIdExist($ID) == 1) {
			$query = mysqli_query($this->db,"SELECT * FROM i_website_social_networks WHERE id = '$ID'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			if($data){
				return $data;
			}else{
	...
	public function iN_UpdateWebsiteSocialSite($userID, $socialSID, $newSocialSite, $newSocialSiteKey, $newSocialSiteStatus, $newSocialSiteSVGCode) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckWebsiteSocialSiteIdExist($socialSID) == 1) {
			$time = time();
			$updateSSite = mysqli_query($this->db, "UPDATE i_website_social_networks SET social_icon = '$newSocialSiteSVGCode', skey = '$newSocialSiteKey' , place_holder = '$newSocialSite' , status = '$newSocialSiteStatus' WHERE id = '$socialSID'") or die(mysqli_error($this->db));
			if($updateSSite){
                return true;
			}else{
				return false;
	...
	public function iN_DeleteWebsiteSocialSite($userID, $ssID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckWebsiteSocialSiteIdExist($ssID) == 1) {
			mysqli_query($this->db, "DELETE FROM i_website_social_networks WHERE id = '$ssID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_ShowWebsiteSocialSitesList(){
		$query = mysqli_query($this->db, "SELECT * FROM i_website_social_networks WHERE status = 'yes' ORDER BY id") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_UpdateApprovePostStatusAuto($userID, $postID, $postOwnerID,  $approveNot) {
		if ($this->iN_CheckUserExist($postOwnerID) == 1 && $this->iN_CheckPostIDExist($postID) == 1) {
			...
			mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
			mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
			mysqli_query($this->db, "UPDATE i_posts SET post_status = '$stat' WHERE post_id = '$postID'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "INSERT INTO i_approve_post_notification(approved_post_id,approved_post_owner_id,approve_status,approve_not,appprove_time)VALUES('$postID','$postOwnerID','1','$approveNot','$time')") or die(mysqli_error($this->db));
			mysqli_query($this->db, "INSERT INTO i_user_notifications(not_post_id, not_not_type,not_time, not_own_iuid, not_iuid)VALUES('$postID','$notType','$time','$postOwnerID','$userID')") or die(mysqli_error($this->db));
			mysqli_query($this->db, "UPDATE i_users SET notification_read_status = '1' WHERE iuid = '$postOwnerID'");
			return true;
		} else {
	...
	public function iN_AllUserForPurchasedPremium($uid, $lastPostID, $showingPost) {
		...
		$morePost = "";
		if ($lastPostID) {
			$morePost = " and X.payment_id <'" . $lastPostID . "' ";
		}
		$data = null;
		$getData = mysqli_query($this->db, "SELECT DISTINCT P.*,X.*,U.*
		FROM i_posts P FORCE INDEX (ixForcePostOwner)
			INNER JOIN i_users U FORCE INDEX (ixForceUser)
			ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3')
			INNER JOIN i_user_payments X FORCE INDEX (ixPayment)
			ON X.payed_post_id_fk = P.post_id
		WHERE X.payer_iuid_fk = '$uid' AND P.post_status IN('1') AND P.who_can_see IN('4') $morePost
		ORDER BY X.payment_id
		DESC LIMIT 1 ") or die(mysqli_error($this->db));
	...
	public function iN_GetAdminUserID(){
		$query = mysqli_query($this->db,"SELECT * FROM i_users WHERE userType = '2' ORDER BY rand() LIMIT 1") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($data['iuid']) ? $data['iuid'] : FALSE;
	}
	/*Approve / Reject / Decline Premium Post*/
	...
	public function iN_GetCategories(){
		$query = mysqli_query($this->db,"SELECT * FROM i_profile_categories WHERE c_status = '1'") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_CheckAndGetSubCat($subCatID){
        $query = mysqli_query($this->db,"SELECT * FROM i_profile_sub_categories WHERE c_fk = '$subCatID' AND sc_status = '1'") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_INSERTUploadedFilesForVerification($uid, $filePath, $tumbnailPath, $fileXPath, $ext) {
		...
		$uploadTime = time();
		$userIP = $_SERVER['REMOTE_ADDR'];
		$query = mysqli_query($this->db, "INSERT INTO i_user_uploads (iuid_fk,uploaded_file_path,upload_tumbnail_file_path,uploaded_x_file_path, uploaded_file_ext,upload_time,ip, upload_type)VALUES('$uid' ,'$filePath','$tumbnailPath','$fileXPath','$fileExtension','$uploadTime','$userIP','verification')") or die(mysqli_error($this->db));
		$ids = mysqli_insert_id($this->db);
		return $ids;
	}
	...
	public function iN_UpdateMercadoPagoDetails($userID, $testID, $liveID, $mercadoCurrency) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_payment_methods SET mercadopago_test_access_id = '$testID', mercadopago_live_access_id = '$liveID', mercadopago_currency ='$mercadoCurrency' WHERE payment_method_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateMercadoPagoMode($userID, $mode) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_payment_methods SET mercadopago_payment_mode = '$mode' WHERE payment_method_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateMercadoPagoStatus($userID, $mode) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_payment_methods SET mercadopago_active_pasive = '$mode' WHERE payment_method_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateFFMPEGDrawTextStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET enable_disable_drawtext = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_GetCategoriesForAdmin(){
		$query = mysqli_query($this->db,"SELECT * FROM i_profile_categories") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_CheckAndGetSubCatForAdmin($subCatID){
        $query = mysqli_query($this->db,"SELECT * FROM i_profile_sub_categories WHERE c_fk = '$subCatID'") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_UpdateSubCategoryStatus($userID, $mod, $iD) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_profile_sub_categories SET sc_status = '$mod' WHERE sc_id = '$iD'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateSubCategoryKey($userID, $sKey, $iD) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_profile_sub_categories SET sc_key = '$sKey' WHERE sc_id = '$iD'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_CreateNewSubCategory($userID, $newSubCatKey, $addToThisCategory) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "INSERT INTO i_profile_sub_categories (sc_key, c_fk, sc_status)VALUES('$newSubCatKey','$addToThisCategory','0')") or die(mysqli_error($this->db));
			if($query){
				$queryGet = mysqli_query($this->db,"SELECT * FROM i_profile_sub_categories WHERE c_fk = '$addToThisCategory' ORDER BY sc_id DESC LIMIT 1") or die(mysqli_error($this->db));
				$data = mysqli_fetch_array($queryGet, MYSQLI_ASSOC);
		        return $data;
			}
		} else {
			return false;
	...
	public function iN_DeleteSubCat($userID,$subCID) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "DELETE FROM i_profile_sub_categories WHERE sc_id = '$subCID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateCategoryStatus($userID, $mod, $iD) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_profile_categories SET c_status = '$mod' WHERE c_id = '$iD'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateCategoryKey($userID, $Key, $iD) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_profile_categories SET c_key = '$Key' WHERE c_id = '$iD'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_DeleteCat($userID,$subCID) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "DELETE FROM i_profile_categories WHERE c_id = '$subCID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_InsertNewProfileCategory($userID, $newCategoryKey) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "INSERT INTO i_profile_categories (c_key,c_status)VALUES('$newCategoryKey','0')") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_GetCategoryFromSubCategory($subCatKey){
       $query = mysqli_query($this->db,"SELECT * FROM i_profile_sub_categories WHERE sc_key = '$subCatKey'") or die(mysqli_error($this->db));
	   $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
	   $catFK = isset($data['c_fk']) ? $data['c_fk'] : NULL;
	   if($catFK){
          $queryGet = mysqli_query($this->db,"SELECT * FROM i_profile_categories WHERE c_id = '$catFK'") or die(mysqli_error($this->db));
		  $data = mysqli_fetch_array($queryGet, MYSQLI_ASSOC);
	...
	public function iN_UpdateUploadStatus($fileID){
       mysqli_query($this->db,"UPDATE i_user_uploads SET upload_status = '1' WHERE upload_id = '$fileID'") or die(mysqli_error($this->db));
	}

	public function iN_CheckLangKeyExist($langKey) {
		$langKey = mysqli_real_escape_string($this->db, $langKey);
		$query = mysqli_query($this->db, "SELECT * FROM i_langs WHERE lang_name = '$langKey' AND lang_status = '1'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) > 0){
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
	...
	public function GetFileIDs(){
	    $getData = mysqli_query($this->db,"SELECT * FROM i_user_uploads WHERE upload_type = 'wall'") or die(mysqli_error($this->db));
	    while ($row = mysqli_fetch_array($getData)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function GetFileIDsPost(){
	    $getData = mysqli_query($this->db,"SELECT * FROM i_posts WHERE post_file <>''") or die(mysqli_error($this->db));
	    while ($row = mysqli_fetch_array($getData)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function CheckFilePosted($hashTag) {
		...
		foreach ($hashtags_list as $ht) {
			// Each tag has to be checked with LIKE alone
			$hashtag_query[] = "FIND_IN_SET(LOWER('$ht'), LOWER(post_file))";
		}
		$hashtag_query = array_unique($hashtag_query, SORT_REGULAR);
		// Make them into AND conditions
		$hashtag_query = implode(' AND ', $hashtag_query);
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$getData = mysqli_query($this->db, "SELECT * FROM i_posts WHERE ($hashtag_query) ORDER BY post_id ") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($getData, MYSQLI_ASSOC)) {
			// Store the result into array
			$data[] = $row;
		}
	...
	public function GetFileTDetail($id){
		$query = mysqli_query($this->db,"SELECT * FROM i_user_uploads WHERE upload_id = '$id'");
		$result = mysqli_fetch_array($query, MYSQLI_ASSOC);
	...
	public function iN_InsertMentionedUsersForComment($userID, $postDetails, $postID, $dataUsername,$userPostOwnerID) {
		...
		if ($pregMatch) {
			...
			$mentionTime = time();
			$mentioned = array_unique($mentioned);
			$mention_sql = mysqli_query($this->db, "INSERT INTO i_mentions (m_uid_fk , m_type , m_post_id_fk,m_user_owner, m_status,mention_type, m_time ) SELECT iuid, NULL, '$postID','$userID', '1', 'comment', '$mentionTime' FROM i_users WHERE i_username IN ('" . implode("','", $mentioned) . "')") or die(mysqli_error($this->db));
			mysqli_query($this->db, "INSERT INTO i_user_notifications(not_post_id, not_not_type,not_time, not_own_iuid, not_iuid) SELECT '$postID','umentioned', '$mentionTime', iuid, '$userID' FROM i_users WHERE i_username IN ('" . implode("','", $mentioned) . "')") or die(mysqli_error($this->db));
	...
	public function iN_AllUserProfilePostsPined($uid) {
		...
		FROM i_friends F
			...
			INNER JOIN i_users U
			ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3') AND F.fr_status IN('me', 'flwr', 'subscriber')
		WHERE P.post_owner_id='$uid' AND P.post_pined = '1' AND P.boosted_status = '0'
		ORDER BY P.post_id
		DESC LIMIT 10 ") or die(mysqli_error($this->db));
...
public function iN_BoostPlans() {
		$query = mysqli_query($this->db, "SELECT * FROM i_boost_post_plans WHERE plan_status = '1'") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function CheckBoostPlanExist($planID) {
		$planID = mysqli_real_escape_string($this->db, $planID);
		$query = mysqli_query($this->db, "SELECT plan_id FROM i_boost_post_plans WHERE plan_id = '$planID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($query) == 1) {
			return true;
		} else {return false;}
	}
    ...
    public function iN_GetBoostPostDetails($boostPlanID){
		$query = mysqli_query($this->db, "SELECT * FROM i_boost_post_plans WHERE plan_id = '$boostPlanID'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) > 0){
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $data;
		}else{
	...
	public function iN_InsertNewWebSocialSite($userID, $newSocialSite, $newSocialSiteKey, $newSocialSiteStatus, $newSocialSiteSVGCode) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			...
			mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
			mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
			$query = mysqli_query($this->db,"INSERT INTO i_website_social_networks(social_icon, skey, place_holder, status)VALUES('$newSocialSiteSVGCode','$newSocialSiteKey','$newSocialSite','$newSocialSiteStatus')") or die(mysqli_error($this->db));
			if($query){
                return true;
			}else{
				return false;
	...
	public function iN_BoostInsert($userID, $boostPostID, $planAmount,$boostPlanID,$viewTime) {
		if($this->iN_CheckUserExist($userID) == '1'){
			$time = time();
			$queryBoost = mysqli_query($this->db, "INSERT INTO i_boosted_posts(iuid_fk, post_id_fk, boost_type, view_count, status,started_at)VALUES('$userID','$boostPostID','$boostPlanID','$viewTime','yes','$time')") or die(mysqli_error($this->db));
		    if($queryBoost){
			    $getLastBoostDetails = mysqli_query($this->db,"SELECT * FROM i_boosted_posts WHERE iuid_fk = '$userID' ORDER BY boost_id DESC LIMIT 1 ") or die(mysqli_error($this->db));
				$data = mysqli_fetch_array($getLastBoostDetails, MYSQLI_ASSOC);
				$boostID = isset($data['boost_id']) ? $data['boost_id'] : NULL;
				$query = mysqli_query($this->db, "INSERT INTO i_user_payments(payer_iuid_fk, payed_post_id_fk, payment_type, payment_time, payment_status, amount)VALUES('$userID','$boostPostID','boostPost','$time', 'ok','$planAmount')") or die(mysqli_error($this->db));
				$queryUser = mysqli_query($this->db,"UPDATE i_users SET wallet_points = wallet_points - '$planAmount' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
				$queryUser = mysqli_query($this->db,"UPDATE i_posts SET boosted_status = '1', boost_id_fk = '$boostID' WHERE post_id = '$boostPostID' AND post_owner_id = '$userID'") or die(mysqli_error($this->db));
			    return true;
			}else{
				return false;
			}
	...
	public function iN_CheckPostBoostedBefore($userID, $postID){
		if($this->iN_CheckUserExist($userID) == '1' && $this->iN_CheckPostIDExist($postID) == '1'){
           $query = mysqli_query($this->db,"SELECT * FROM i_boosted_posts WHERE post_id_fk = '$postID' AND iuid_fk = '$userID'") or die(mysqli_error($this->db));
		   if(mysqli_num_rows($query) > 0){
              return true;
		   }else{
			  return false;
	...
	public function iN_GetBoostedPostDetails($postID){
       $query = mysqli_query($this->db,"SELECT * FROM i_boosted_posts WHERE post_id_fk = '$postID'") or dei(mysqli_error($this->db));
	   if(mysqli_num_rows($query) > 0){
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $data;
		}else{
	...
	public function iN_AllUserForBoostedPosts($uid, $lastPostID, $showingPost) {
        ...
        $morePost = !empty($lastPostID) ? "AND P.post_id < '$lastPostID'" : "";
        ...
        $query = "SELECT P.*, U.*
                  ...
                  AND P.boosted_status = '1' $morePost
                  AND U.uStatus IN ('1', '3')
                  AND F.fr_status IN ('me')
                  ORDER BY P.post_id DESC
                  LIMIT $showingPosts";
	...
	public function iN_CheckBoostIDExist($userID, $boostPostID){
       $query = mysqli_query($this->db, "SELECT * FROM i_boosted_posts WHERE boost_id = '$boostPostID' AND iuid_fk = '$userID'") or die(mysqli_error($this->db));
	   if(mysqli_num_rows($query) > 0){
          return true;
	   }else{
		  return false;
	...
	public function iN_UpdateBoosPostStatus($userID, $bPostID, $bpStatus){
		if($this->iN_CheckUserExist($userID) == '1' && $this->iN_CheckBoostIDExist($userID, $bPostID) == '1'){
            $query = mysqli_query($this->db,"UPDATE i_boosted_posts SET status = '$bpStatus' WHERE boost_id = '$bPostID' AND iuid_fk = '$userID'") or die(mysqli_error($this->db));
			return true;
		}else{
			return false;
		}
	...
	public function iN_CheckBoostedPostSeenBefore($userID, $ip, $boostID){
		$query = mysqli_query($this->db, "SELECT * FROM i_boosted_post_seen_counter WHERE bp_id_fk = '$boostID' AND iuid_fk = '$userID' AND ip = '$ip'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) == 0){
		   return true;
		}else{
		   return false;
	...
	public function iN_BoostPostSeenCounter($userID, $boostID, $ip){
       if($this->iN_CheckuserExist($userID) == '1' && $this->iN_CheckBoostedPostSeenBefore($userID, $ip, $boostID) == '1'){
		   $time = time();
           $query = mysqli_query($this->db, "INSERT INTO i_boosted_post_seen_counter(iuid_fk, bp_id_fk, ip, bp_seen_time)VALUES('$userID','$boostID','$ip','$time')") or die(mysqli_error($this->db));
		   return true;
	   }else{
		return false;
	   }
	...
	public function iN_ShowBoostedPost($userID, $showingPost) {
		$showingPosts = mysqli_real_escape_string($this->db, $showingPost);
		$data = null;
		$getData = mysqli_query($this->db, "SELECT P.*, U.*, B.*
			FROM i_posts P
			INNER JOIN i_users U ON P.post_owner_id = U.iuid
			...
			WHERE P.boosted_status = '1'
			  AND P.post_owner_id != '$userID'
			  AND U.uStatus IN ('1', '3')
			  AND F.fr_status NOT IN ('flwr', 'subscriber')
			ORDER BY RAND()
			LIMIT 1") or die(mysqli_error($this->db));
	...
	public function iN_CheckHaveBoostedPostAllTheSite(){
		$countBoostedPosts = mysqli_query($this->db, "SELECT COUNT(*) AS countBoostedPosts FROM i_boosted_posts WHERE status = 'yes'") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($countBoostedPosts, MYSQLI_ASSOC);
		return isset($row['countBoostedPosts']) ? $row['countBoostedPosts'] : '0';

	}
	/*Check User Have Boosted Post*/
	public function iN_CheckHaveBoostedPost($userID){
		if($this->iN_CheckuserExist($userID) == '1'){
			$countBoostedPost = mysqli_query($this->db, "SELECT COUNT(*) AS countBoostedPost FROM i_boosted_posts WHERE iuid_fk = '$userID' AND status IN('no','yes')") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($countBoostedPost, MYSQLI_ASSOC);
			return isset($row['countBoostedPost']) ? $row['countBoostedPost'] : '0';
		}else{
			return false;
		}
	}
	...
	public function iN_TotalBoostedPost($userID) {
        if ($this->iN_CheckIsAdmin($userID) == 1) {
            $q = mysqli_query($this->db, "SELECT COUNT(*) AS totalBoostedPost
                FROM i_boosted_posts BP
                INNER JOIN i_posts P ON BP.post_id_fk = P.post_id
                WHERE EXISTS (SELECT 1 FROM i_posts WHERE post_id = BP.post_id_fk)
            ") or die(mysqli_error($this->db));
            $row = mysqli_fetch_array($q, MYSQLI_ASSOC);
            if ($row) {
                return isset($row['totalBoostedPost']) ? $row['totalBoostedPost'] : '0';
            } else {
                return false;
            }
        }
	...
	public function iN_CountSeenBoostedPostbyID($userID,$boostedPostID){
		if($this->iN_CheckuserExist($userID) == '1'){
			$countBoostedPost = mysqli_query($this->db, "SELECT COUNT(*) AS countBoostedPostbyID FROM i_boosted_post_seen_counter WHERE bp_id_fk = '$boostedPostID'") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($countBoostedPost, MYSQLI_ASSOC);
			return isset($row['countBoostedPostbyID']) ? $row['countBoostedPostbyID'] : '0';
		}else{
			return false;
		}
	}
	...
	public function iN_UpdateBoostedPostStatus($userPostOwnerID, $boostID){
		$query = mysqli_query($this->db,"UPDATE i_boosted_posts SET status = 'no' WHERE boost_id = '$boostID' AND iuid_fk = '$userPostOwnerID'") or die(mysqli_error($this->db));
	}
	...
	public function iN_ShowAllBoostedPost($userID, $paginationLimit, $page) {
		...
		if($this->iN_CheckIsAdmin($userID) == 1){
			...
			FROM  i_posts P
				INNER JOIN i_users U
				ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3')
				INNER JOIN i_boosted_posts B
				ON P.boost_id_fk = B.boost_id
			WHERE P.boosted_status = '1'
			ORDER BY B.boost_id DESC LIMIT $start_from, $paginationLimit") or die(mysqli_error($this->db));
	...
	public function iN_CheckBoostExist($boostPostID){
		$query = mysqli_query($this->db, "SELECT * FROM i_boosted_posts WHERE boost_id = '$boostPostID'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) > 0){
		   return true;
		}else{
		   return false;
	...
	public function iN_UpdateBoostPostStatus($userID, $mode, $bgID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckBoostExist($bgID) == 1) {
			mysqli_query($this->db, "UPDATE i_boosted_posts SET status = '$mode' WHERE boost_id = '$bgID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_GetBoostedDetailsByID($postID){
		$query = mysqli_query($this->db,"SELECT * FROM i_boosted_posts WHERE boost_id = '$postID'") or dei(mysqli_error($this->db));
		if(mysqli_num_rows($query) > 0){
			 $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			 return $data;
		 }else{
	...
	public function iN_DeleteBoostedPost($userID, $boostID){
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			...
			mysqli_query($this->db, "UPDATE i_posts SET boost_id_fk = '0' WHERE boost_id_fk = '$boostID'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "DELETE FROM i_boosted_posts WHERE boost_id = '$boostID'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "DELETE FROM i_boosted_posts WHERE boost_id = '$boostID'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "DELETE FROM i_user_payments WHERE payed_post_id_fk = '$boostID' AND payment_type = 'boostPost'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_TotalBoostEarnings($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT SUM(amount) AS boostEarn FROM i_user_payments WHERE payment_status = 'ok' AND payment_type = 'boostPost'") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return isset($row['boostEarn']) ? $row['boostEarn'] : '0.00';
		}
	}
    /*Total Boost Earnings Weekly*/
	public function iN_WeeklyTotalBoostEarning() {
		$query = mysqli_query($this->db, "SELECT SUM(amount) AS WeekTotalEarnBoost FROM i_user_payments WHERE  payment_status = 'ok' AND payment_type = 'boostPost' AND WEEK(FROM_UNIXTIME(payment_time)) = WEEK(NOW())") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($row['WeekTotalEarnBoost']) ? $row['WeekTotalEarnBoost'] : '0.00';
	}
	/*Total Boost Earnings Current Day*/
	public function iN_CurrentDayTotalBoostEarning() {
		$query = mysqli_query($this->db, "SELECT SUM(amount) AS DayTotalEarnBoost FROM i_user_payments WHERE payment_status = 'ok' AND payment_type = 'boostPost' AND DAY(FROM_UNIXTIME(payment_time)) = DAY(CURDATE()) ") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($row['DayTotalEarnBoost']) ? $row['DayTotalEarnBoost'] : '0.00';
	}
	/*Total Boost Earnings Current Month*/
	public function iN_CurrentMonthTotalBoostEarning() {
		$query = mysqli_query($this->db, "SELECT SUM(amount) AS MonthTotalEarnBoost FROM i_user_payments WHERE payment_status = 'ok' AND payment_type = 'boostPost' AND MONTH(FROM_UNIXTIME(payment_time)) = MONTH(CURDATE()) ") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($row['MonthTotalEarnBoost']) ? $row['MonthTotalEarnBoost'] : '0.00';
	}
	/*Latest 5 Subscriptions List*/
	public function iN_SubscriptionsListData($userID, $paginationLimit, $page) {
		...
		if($this->iN_CheckIsAdmin($userID) == 1){
			$query = mysqli_query($this->db, "SELECT
				...
				INNER JOIN i_users U
				ON S.subscribed_iuid_fk = U.iuid AND U.uStatus IN('1','3')
				WHERE S.status = 'active' ORDER BY S.subscription_id DESC LIMIT $start_from, $paginationLimit
				") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_CalculateAllSubscriptions() {
		$query = mysqli_query($this->db, "SELECT COUNT(*) AS allSubs FROM i_user_subscriptions ") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($row['allSubs']) ? $row['allSubs'] : '0';
	}
	/*Calculate All Active Subscriptions*/
	public function iN_CalculateAllActiveSubscriptions($type) {
		$query = mysqli_query($this->db, "SELECT COUNT(*) AS allSubs FROM i_user_subscriptions WHERE status = '$type'") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($row['allSubs']) ? $row['allSubs'] : '0';
	}
	...
	public function iN_BoostPlanList() {
		$query = mysqli_query($this->db, "SELECT * FROM i_boost_post_plans ") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_UpdateBoostPlanStatus($userID, $mod, $planID) {
		...
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckBoostPlanExist($planID) == 1) {
			mysqli_query($this->db, "UPDATE  i_boost_post_plans SET plan_status = '$mod' WHERE plan_id = '$planID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_DeleteBoostPlanFromData($userID, $planID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckBoostPlanExist($planID) == 1) {
			mysqli_query($this->db, "DELETE FROM i_boost_post_plans WHERE plan_id = '$planID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateBoostPlanFromID($userID, $planKey, $planViewtime, $planAmount, $planSVGIcon, $plandID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckBoostPlanExist($plandID)) {
			mysqli_query($this->db, "UPDATE i_boost_post_plans SET plan_name_key = '$planKey', plan_amount = '$planAmount', view_time = '$planViewtime', plan_icon = '$planSVGIcon' WHERE plan_id = '$plandID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_InsertNewBOOSTPlan($userID, $planKey, $planViewTime, $planAmount, $planIcon) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "INSERT INTO i_boost_post_plans(plan_name_key,plan_icon, plan_amount,view_time,plan_status)VALUES('$planKey','$planIcon','$planAmount','$planViewTime','0')") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UserTotalTransactions($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT COUNT(*) AS totalTransactions FROM i_user_payments") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return isset($row['totalTransactions']) ? $row['totalTransactions'] : '0';
		}
	}
	/*Latest 5 Subscriptions List*/
		...
		if($this->iN_CheckIsAdmin($userID) == 1){
			$query = mysqli_query($this->db, "
                ...
                FROM
                    i_users U FORCE INDEX(ixForceUser)
                INNER JOIN
                    i_user_payments P FORCE INDEX(ixPayment)
                ON
                    P.payer_iuid_fk = U.iuid AND U.uStatus IN ('1','3')
                WHERE
                    P.payment_type IN ('post','live_stream','tips','live_gift','videoCall','boostPost','point','profile','product','frame')
                ORDER BY
                    P.payment_id DESC
                LIMIT
                    $start_from, $paginationLimit
            ") or die(mysqli_error($this->db));
	...
	public function iN_CalculateTypePayment($type) {
			$query = mysqli_query($this->db, "SELECT SUM(amount) AS calculate FROM i_user_payments WHERE payment_type = '$type'") or die(mysqli_error($this->db));
			$result = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return isset($result['calculate']) ? $result['calculate'] : '0';
	}
	/*Current Month Earn Calculate*/
	public function iN_CalculatePaymentSpecific($type) {
		$query = mysqli_query($this->db, "SELECT SUM(admin_earning) AS calculate FROM i_user_payments WHERE payment_type = '$type'") or die(mysqli_error($this->db));
		$result = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($result['calculate']) ? $result['calculate'] : '0';
    }
	/*INSERT UPLOADED FILES FROM UPLOADS TABLE*/
	public function iN_INSERTUploadedScreenShotForPaymentComplete($uid, $filePath, $tumbnailPath, $fileXPath, $ext) {
		...
		$uploadTime = time();
		$userIP = $_SERVER['REMOTE_ADDR'];
		$query = mysqli_query($this->db, "INSERT INTO i_user_uploads (iuid_fk,uploaded_file_path,upload_tumbnail_file_path,uploaded_x_file_path, uploaded_file_ext,upload_time,ip, upload_type)VALUES('$uid' ,'$filePath','$tumbnailPath','$fileXPath','$fileExtension','$uploadTime','$userIP','bankPayment')") or die(mysqli_error($this->db));
		$ids = mysqli_insert_id($this->db);
		return $ids;
	}
	/*Insert  A New Verification Request*/
	public function iN_InsertNewBankPaymentVerificationRequest($userID, $cardIDPhoto, $planAmount, $planPoint, $planID) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			$time = time();
			$InsertQuery = mysqli_query($this->db, "INSERT INTO i_bank_payments(iuid_fk,screen_photo,request_time)VALUES('$userID','$cardIDPhoto','$time')") or die(mysqli_error($this->db));
			$query = mysqli_query($this->db, "INSERT INTO i_user_payments(payer_iuid_fk,payment_type,payment_option,payment_time,payment_status, amount, credit_plan_id,bank_payment_image)VALUES('$userID', 'point','bank', '$time', 'pending', '$planAmount','$planID','$cardIDPhoto')") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_GetPaymentDetailsByID($paymentID, $userID){
		if($this->iN_CheckIsAdmin($userID) == 1){
			$query = mysqli_query($this->db,"SELECT * FROM i_user_payments WHERE payment_id = '$paymentID'") or dei(mysqli_error($this->db));
			if(mysqli_num_rows($query) > 0){
				$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
				return isset($data) ? $data : FALSE;
			}else{
	...
	public function iN_GetImageByID($base_url, $userID , $ImageID){
		if($this->iN_CheckIsAdmin($userID) == 1){
			...
			if(mysqli_num_rows($query) == '1'){
                $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
				$imageFile = isset($data['uploaded_file_path']) ? $data['uploaded_file_path'] : NULL;
				$checkS3Status = mysqli_query($this->db, "SELECT * FROM i_configurations WHERE configuration_id = '1'") or die(mysqli_error($this->db));
				$s3 = mysqli_fetch_array($checkS3Status, MYSQLI_ASSOC);
				$s3Status = isset($s3['s3_status']) ? $s3['s3_status'] : '0';
				$wasStatus = isset($s3['was_status']) ? $s3['was_status'] : '0';
				...
				if ($s3Status == 1) {
					$dataImage = 'https://' . $s3['s3_bucket'] . '.s3.' . $s3['s3_region'] . '.amazonaws.com/' . $imageFile;
				}else if($wasStatus == 1){
					$dataImage = 'https://' . $s3['was_bucket'] . '.s3.' . $s3['was_region'] . '.wasabisys.com/' . $imageFile;
				}else if($oceanStatus == 1){
					$dataImage = 'https://'.$s3['ocean_space_name'].'.'.$s3['ocean_region'].'.digitaloceanspaces.com/'. $imageFile;
				} else {
					$dataImage = $base_url . $imageFile;
				}
				return $dataImage;
			}
		}
	...
	public function iN_InsertApprove($userID, $payerID, $planID, $imageID,$paymentIDD){
		if($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckPlanExist($planID) == 1 && $this->iN_CheckUserExist($userID) == 1){
            mysqli_query($this->db,"UPDATE i_bank_payments SET request_status = 'yes' WHERE iuid_fk = '$payerID' AND screen_photo = '$imageID'") or die(mysqli_error($this->db));
			mysqli_query($this->db,"UPDATE i_user_payments SET payment_status = 'ok' WHERE payment_id = '$paymentIDD' AND payer_iuid_fk = '$payerID' AND bank_payment_image = '$imageID'") or die(mysqli_error($this->db));
			$planData = $this->GetPlanDetails($planID);
			$planCreditAmount = isset($planData['plan_amount']) ? $planData['plan_amount'] : NULL;
			if($planCreditAmount){
				mysqli_query($this->db,"UPDATE i_users SET `wallet_points` = `wallet_points` + '$planCreditAmount' WHERE iuid = '$payerID'") or die(mysqli_error($this->db));
			}
            return true;
		}
	}
	...
	public function iN_DeclineBankPaymentRequest($userID, $payerID, $planID, $imageID,$paymentIDD){
		if($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckPlanExist($planID) == 1 && $this->iN_CheckUserExist($userID) == 1){
            mysqli_query($this->db, "DELETE FROM i_user_payments WHERE payment_id = '$paymentIDD'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "DELETE FROM i_bank_payments WHERE iuid_fk = '$payerID' AND screen_photo = '$imageID'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "DELETE FROM i_user_uploads WHERE iuid_fk = '$payerID' AND upload_id = '$imageID'") or die(mysqli_error($this->db));
			return true;
		}
	}
	/*User Total Point Payments*/
	public function iN_UserTotalTransactionsPoints($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT COUNT(*) AS totalTransactions FROM i_user_payments WHERE payment_type IN('point')") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return isset($row['totalTransactions']) ? $row['totalTransactions'] : '0';
		}
	}
	...
	public function iN_TransactionsListPoints($userID, $paginationLimit, $page) {
		...
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT P.*, U.*
				FROM i_users U
				INNER JOIN i_user_payments P
				ON P.payer_iuid_fk = U.iuid
				WHERE U.uStatus IN ('1', '3') AND P.payment_type = 'point'
	...
	public function iN_UpdateDetectLanguageStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET auto_detect_language_status = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateEmailSendStatusForCPP($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET send__email = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateBankPaymentPagoStatus($userID, $mode) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_payment_methods SET bank_payment_status = '$mode' WHERE payment_method_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateBankPaymentDetails($userID, $percentageFee, $fixedCharge, $bankDescription) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_payment_methods SET bank_payment_percentage_fee = '$percentageFee', bank_payment_fixed_charge = '$fixedCharge', bank_payment_details ='$bankDescription' WHERE payment_method_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_FrameListFromAdmin() {
		$query = mysqli_query($this->db, "SELECT * FROM i_frames") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_FrameListFromProfile() {
		$query = mysqli_query($this->db, "SELECT * FROM i_frames WHERE f_frame_status = '1'") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query)) {
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function iN_UpdateFramePlanFromID($userID, $frameAmount, $frameFile,$frameID) {
		if ($this->iN_CheckIsAdmin($userID) == 1 && $this->CheckFramePlanExist($frameID)) {
			mysqli_query($this->db, "UPDATE i_frames SET f_price = '$frameAmount', f_file = '$frameFile' WHERE f_id = '$frameID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_CheckFramePurchased($userID, $frameID){
	    $query = mysqli_query($this->db,"SELECT * FROM i_user_frames WHERE f_purchased_frame_id = '$frameID'") or die(mysqli_error($this->db));
	    if(mysqli_num_rows($query) == 1){
	        return true;
	    }else{
	        return false;
	...
	public function iN_PurchaseFrame($userID, $purchasedID, $frameID, $onePointEqual){
        ...
        if ($this->iN_CheckuserExist($userID) == '1' && $this->iN_CheckuserExist($purchasedID) == '1') {
            mysqli_query($this->db, "UPDATE i_users SET user_frame = '$frameImagePath' WHERE iuid = '$purchasedID'") or die(mysqli_error($this->db));
            mysqli_query($this->db, "UPDATE i_users SET wallet_points = wallet_points - $framePrice WHERE iuid = '$userID'") or die(mysqli_error($this->db));

            if ($this->iN_CheckFramePurchased($userID, $frameID) == 1) {
                mysqli_query($this->db, "UPDATE i_user_frames SET f_total_purchased_this_frame = f_total_purchased_this_frame + 1 WHERE f_purchased_frame_id = '$frameID'") or die(mysqli_error($this->db));
            } else {
                mysqli_query($this->db, "INSERT INTO i_user_frames(f_puchased_user_id,f_purchaser_user_id,f_purchased_time,f_purchase_price,f_purchased_frame_id,f_total_purchased_this_frame)
                VALUES('$purchasedID','$userID','$purchaseTime','$framePrice','$frameID','1')") or die(mysqli_error($this->db));
            }

            mysqli_query($this->db, "INSERT INTO i_user_payments(payer_iuid_fk, payed_iuid_fk, purchased_frame_id, payment_type, payment_time, payment_status, amount, admin_earning)
            VALUES('$userID','$purchasedID','$frameID','frame','$purchaseTime','ok','$frameAmount','$frameAmount')") or die(mysqli_error($this->db));
	...
	public function iN_CheckUserHasThisFrame($userID, $frameID){
	    $userID = mysqli_real_escape_string($this->db, $userID);
	    $frameID = mysqli_real_escape_string($this->db, $frameID);
	    if($this->CheckFramePlanExist($frameID) == '1' && $this->iN_CheckuserExist($userID) == '1'){
	         $query = mysqli_query($this->db,"SELECT * FROM i_user_frames WHERE f_puchased_user_id = '$userID' AND f_purchased_frame_id = '$frameID'") or die(mysqli_error($this->db));
	         if(mysqli_num_rows($query) > 0){
	             return true;
	         }else{
	             return false;
	...
	public function iN_FrameListWithCheckFromUserDashboard($userID) {
        $userID = mysqli_real_escape_string($this->db, $userID);
        $query = mysqli_query($this->db, "SELECT i_frames.*, i_user_frames.f_purchased_frame_id IS NOT NULL AS purchased FROM i_frames LEFT JOIN i_user_frames ON i_frames.f_id = i_user_frames.f_purchased_frame_id AND i_user_frames.f_puchased_user_id = '$userID' WHERE i_frames.f_frame_status = '1'") or die(mysqli_error($this->db));
    ...
    public function iN_CheckUnsubscribeStatus($userID, $profileID){
        if($this->iN_CheckuserExist($userID) == '1' && $this->iN_CheckuserExist($profileID) == '1'){
            $query = mysqli_query($this->db,"SELECT * FROM i_user_subscriptions WHERE iuid_fk = '$userID' AND subscribed_iuid_fk = '$profileID' AND status = 'inactive' AND in_status = '1' AND finished = '1'AND UNIX_TIMESTAMP(plan_period_end) > UNIX_TIMESTAMP(NOW()) ORDER BY subscription_id DESC LIMIT 1") or die(mysqli_error($this->db));
            if(mysqli_num_rows($query) > 0){
                $getUnSubscribeTime = mysqli_query($this->db, "SELECT plan_period_end FROM i_user_subscriptions WHERE iuid_fk = '$userID' AND subscribed_iuid_fk = '$profileID' AND status = 'inactive' AND in_status = '1' AND finished = '1'AND UNIX_TIMESTAMP(plan_period_end) > UNIX_TIMESTAMP(NOW()) ORDER BY subscription_id DESC LIMIT 1") or die(mysqli_error($this->db));
                if(mysqli_num_rows($getUnSubscribeTime) > 0){
                    $row = mysqli_fetch_array($getUnSubscribeTime, MYSQLI_ASSOC);
                    return isset($row['plan_period_end']) ? $row['plan_period_end'] : FALSE;
                }else{
                    return false;
                }
            }else{
    ...
    public function iN_UpdateFrame($userID, $frameID){
        ...
        if($this->CheckFramePlanExist($frameID) == '1' && $this->iN_CheckuserExist($userID) == '1' && $this->iN_CheckUserHasThisFrame($userID, $frameID)){
            $frameData = $this->GetFramePlanDetails($frameID);
	        $frameImagePath = isset($frameData['f_file']) ? $frameData['f_file'] : NULL;
	        mysqli_query($this->db,"UPDATE i_users SET user_frame = '$frameImagePath' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
	        return true;
        }
    }
    /*Calculate All Questions*/
	public function iN_CalculateAllUnreadQuestions() {
		$query = mysqli_query($this->db, "SELECT COUNT(*) AS psts FROM i_contacts WHERE contact_read_status = '0'") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($row['psts']) ? $row['psts'] : '0';
	}
	/*Set Blur*/
	public function iN_CheckMuteBlurOwner($userID,$chatIDfk){
	    $query = mysqli_query($this->db,"SELECT video_muted FROM i_video_call WHERE video_muted = '$userID' AND chat_id_fk = '$chatIDfk'") or die(mysqli_error($this->db));
	    $data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return isset($data['video_muted']) ? $data['video_muted'] : false;
	...
	public function iN_UpdateMuteUnmute($userID,$chatIDfk){
	    if ($this->iN_CheckUserExist($userID) == 1) {
	        if($this->iN_VideoBlurCheck($chatIDfk) == 'not'){
	            $query = mysqli_query($this->db,"UPDATE i_video_call SET video_muted = '$userID' WHERE chat_id_fk = '$chatIDfk'") or die(mysqli_error($this->db));
	        }else{
	            if($this->iN_CheckMuteOwner($userID,$chatIDfk) == '1'){
	                $query = mysqli_query($this->db,"UPDATE i_video_call SET video_muted = NULL WHERE chat_id_fk = '$chatIDfk'") or die(mysqli_error($this->db));
	            }
	        }
	        return true;
	    }
	...
	public function iN_VideoBlurCheck($chatIDfk){
	    $query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE chat_id_fk = '$chatIDfk' AND video_muted IS NOT NULL") or die(mysqli_error($this->db));
	    if(mysqli_num_rows($query) == '1'){
	        return 'ok';
	    }else{
	        return 'not';
	...
	public function iN_CheckRowExist($dataRow){
	    $sql = mysqli_query($this->db,"SHOW COLUMNS FROM i_configurations LIKE '$dataRow'") or die(mysqli_error($this->db));
	...
	public function iN_ChangeColor($userID, $dataRow, $color){
	    if($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckRowExist($dataRow) == '1'){
	        $query = mysqli_query($this->db,"UPDATE i_configurations SET $dataRow = '$color' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
	        return true;
	    }else{
	        return false;
	    }
	...
	public function iN_UpdateDefaultColor($userID, $dataRow){
	    if($this->iN_CheckIsAdmin($userID) == 1 && $this->iN_CheckRowExist($dataRow) == '1'){
	        $query = mysqli_query($this->db,"UPDATE i_configurations SET $dataRow = NULL WHERE configuration_id = '1'") or die(mysqli_error($this->db));
	        return true;
	    }else{
	        return false;
	    }
	...
	public function getTotalCurrentOnlineUsers() {
        $query = "SELECT COUNT(*) AS online_user_count FROM i_users WHERE last_login_time >= UNIX_TIMESTAMP(NOW() - INTERVAL 1 MINUTE)";
        ...
        return isset($row['online_user_count']) ? $row['online_user_count'] : '0';
    }
    public function iN_GetTotalHotPosts($uid, $showingPost, $showingTrendPostLimitDay) {
        $uid = mysqli_real_escape_string($this->db, $uid);
        $showingPosts = mysqli_real_escape_string($this->db, $showingPost);
        $showingTrendPostLimitDay = mysqli_real_escape_string($this->db, $showingTrendPostLimitDay);
        $data = null;
        $query = mysqli_query($this->db, "SELECT p.*, u.*, IFNULL(SUM(c.comment_count), 0) AS total_comments, IFNULL(SUM(l.like_count), 0) AS total_likes
            FROM i_posts p
            ...
            WHERE p.post_created_time >= UNIX_TIMESTAMP(NOW() - INTERVAL $showingTrendPostLimitDay DAY)
            GROUP BY p.post_id
            ...
            LIMIT $showingPosts") or die(mysqli_error($this->db));
   ...
   public function iN_FriendsActivity($uid , $showNumber, $avtivityTime) {
    ...
    public function iN_InsertPostActivity($userID, $activityType,$postID, $time){
        $query = mysqli_query($this->db,"INSERT INTO i_user_activity(uid_fk, post_id, activity_type, activity_time)VALUES('$userID','$postID','newPost','$time')") or die(mysqli_error($this->db));
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function iN_InsertPostLikeActivity($userID, $activityType,$postID, $time){
        $query = mysqli_query($this->db,"INSERT INTO i_user_activity(uid_fk, post_id, activity_type, activity_time)VALUES('$userID','$postID','$activityType','$time')") or die(mysqli_error($this->db));
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function iN_InsertFollowActivity($userID, $activityType,$followedUserID, $time){
        $query = mysqli_query($this->db,"INSERT INTO i_user_activity(uid_fk, fr_id, activity_type, activity_time)VALUES('$userID','$followedUserID','$activityType','$time')") or die(mysqli_error($this->db));
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function iN_DeletePostActivity($userID, $postID){
        $query = mysqli_query($this->db, "DELETE FROM  i_user_activity WHERE post_id = '$postID' AND uid_fk = '$userID' AND activity_type = 'newPost'");
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function iN_DeletePostLikeActivity($userID, $postID){
        $query = mysqli_query($this->db, "DELETE FROM  i_user_activity WHERE post_id = '$postID' AND uid_fk = '$userID' AND activity_type = 'postLike'");
        if ($query) {
            return true;
        } else {
            return false;
    ...
    public function iN_DeleteFollowActivity($userID, $frID){
        $query = mysqli_query($this->db, "DELETE FROM  i_user_activity WHERE fr_id = '$postID' AND uid_fk = '$userID' AND activity_type = 'userFollow'");
        if ($query) {
            return true;
        } else {
            return false;
	...
	public function iN_UpdateSubscriptionType($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET unsubscribe_style = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateAutoFollowAdminStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_configurations SET auto_follow_admin = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
    ...
    public function iN_UpdatePaymentSuccessStatusAmount($userID, $paymentID, $amount){
		if ($this->iN_CheckUserExist($userID) == '1') {
		    $query = mysqli_query($this->db, "UPDATE i_user_payments SET payment_status = 'pending', amount = '$amount' WHERE payment_id = '$paymentID'") or die(mysqli_error($this->db));
            return true;
		}
	}
	/*Total Admin Premium Earnings*/
	public function iN_TotalPremiumEarningsNetPoint($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT SUM(amount) AS premEarn FROM i_user_payments WHERE payment_status = 'ok' AND credit_plan_id IS NOT NULL") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return isset($row['premEarn']) ? $row['premEarn'] : '0.00';
		}
	}
	...
	public function iN_AiUsed($userID, $credit) {
		if ($this->iN_CheckUserExist($userID) == '1') {
			$query = mysqli_query($this->db, "SELECT wallet_points FROM i_users WHERE iuid = '$userID'") or die(mysqli_error($this->db));
			$row = mysqli_fetch_array($query);
			$wallet = isset($row['wallet_points']) ? $row['wallet_points'] : 0;
	...
	public function iN_UpdateAiGeneratorStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET open_ai_status = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
    ...
    public function iN_UpdateAiAPIData($userID, $aiKey, $aiPerAmount) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET openai_api_key = '$aiKey', per_ai_use_credit = '$aiPerAmount' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
    ...
    public function iN_DeleteMyUserAccount($userID, $deleteUserID) {

        if ($userID == $deleteUserID && $this->iN_CheckUserExist($deleteUserID) == 1) {
            $safeID = mysqli_real_escape_string($this->db, $deleteUserID);

            $queries = [
                "DELETE FROM i_chat_conversations WHERE user_one = '$safeID' OR user_two = '$safeID'",
                "DELETE FROM i_chat_users WHERE user_one = '$safeID' OR user_two = '$safeID'",
                "DELETE FROM i_comment_reports WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_friends WHERE fr_one = '$safeID' OR fr_two = '$safeID'",
                "DELETE FROM i_posts WHERE post_owner_id = '$safeID'",
                "DELETE FROM i_post_comments WHERE comment_uid_fk = '$safeID'",
                "DELETE FROM i_post_comment_likes WHERE c_like_iuid_fk = '$safeID'",
                "DELETE FROM i_post_likes WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_post_reports WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_saved_posts WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_sessions WHERE session_uid = '$safeID'",
                "DELETE FROM i_user_avatars WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_user_blocks WHERE blocker_iuid = '$safeID' OR blocked_iuid = '$safeID'",
                "DELETE FROM i_user_conversation_uploads WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_user_covers WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_user_notifications WHERE not_iuid = '$safeID' OR not_own_iuid = '$safeID'",
                "DELETE FROM i_user_subscribe_plans WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_user_subscriptions WHERE iuid_fk = '$safeID' OR subscribed_iuid_fk = '$safeID'",
                "DELETE FROM i_user_uploads WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_verification_requests WHERE iuid_fk = '$safeID'",
                "DELETE FROM i_users WHERE iuid = '$safeID'"
            ];
    ...
    public function iN_VerifyCurrentPassword($userID, $password) {
        $userID = mysqli_real_escape_string($this->db, $userID);
        $password = mysqli_real_escape_string($this->db, $password);
        $hashedPassword = sha1(md5($password));

        $check = mysqli_query($this->db, "SELECT iuid FROM i_users WHERE iuid = '$userID' AND i_password = '$hashedPassword'") or die(mysqli_error($this->db));
