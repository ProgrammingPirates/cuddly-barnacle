...
if ($success) {
    ...
    if(mysqli_num_rows($checkUserExist) == 0 && isset($TwitterAccountEmail)){
        ...
        $register = mysqli_query($db,"INSERT INTO i_users(i_user_fullname, i_user_email, user_gender, user_avatar, i_username, registered,i_password, login_with,lang, email_verify_status)VALUES('$TwitterAccountFullName','$TwitterAccountEmail','male','$TwitterAccountProfileImage','$TwitterAccountRegisterUserName','$time','$generatePassword','twitter','$defaultLanguage','yes')") or die(mysqli_error($db));
        if($register){
