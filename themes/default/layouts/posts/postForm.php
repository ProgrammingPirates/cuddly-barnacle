...
$userAvatar = isset($userAvatar) ? $userAvatar : ($base_url.'uploads/avatars/no_gender.png');
$userFullName = isset($userFullName) ? $userFullName : '';
$userWhoCanSeePost = isset($userWhoCanSeePost) ? $userWhoCanSeePost : 1;
$activeWhoCanSee = isset($activeWhoCanSee) ? $activeWhoCanSee : '';
?>
<div class="i_postFormContainer">
    ...
    <div class="i_postLiveStreaming flex_ tabing">
        <?php if($logedIn == '1' && $paidLiveStreamingStatus == '1' && $feesStatus == '2'){?>
            <div class="i_live_ cNLive flex_ tabing_non_justify transition" data-type="paidLive"><?php echo html_entity_decode($iN->iN_SelectedMenuIcon('40'));?><?php echo iN_HelpSecure($LANG['paid_live_streamings']);?></div>
        <?php }?>
        <?php if($logedIn == '1' && $freeLiveStreamingStatus == '1'){?>
            <div class="i_live_ cNLive flex_ tabing_non_justify transition" data-type="freeLive"><?php echo html_entity_decode($iN->iN_SelectedMenuIcon('115'));?><?php echo iN_HelpSecure($LANG['free_live_streams']);?></div>
        <?php }?>
    </div>
    <?php }?>
    ...
    <div class="i_post_form transition aft">
        ...
        <div class="i_post_form_textarea">
            <textarea name="postText" id="newPostT" maxlength="<?php echo iN_HelpSecure($availableLength);?>" class="comment commenta newPostT" placeholder="<?php echo iN_HelpSecure($LANG['write_message_add_photo_or_video']);?>"></textarea>
        </div>
    </div>

    <?php if ($userWhoCanSeePost === '4' || $userWhoCanSeePost === 4) { ?>
    <div class="point_input_wrapper">
        <input type="text" name="point" id="point" class="pointIN" onkeypress="return event.charCode == 46 || (event.charCode >= 48 && event.charCode <= 57)" placeholder="<?php echo iN_HelpSecure($LANG['write_points']);?>">
        <div class="box_not box_not_padding_left"><?php echo iN_HelpSecure($LANG['point_wanted']);?></div>
    </div>
    <?php } ?>
    ...
    <form id="tuploadform" class="options-form" method="post" enctype="multipart/form-data" action="<?php echo iN_HelpSecure($base_url) . 'requests/request.php';?>">
        <div class="i_uploaded_iv nonePoint">
            ...
            <div class="i_uploading_not"><?php echo iN_HelpSecure($LANG['uploading_please_wait']);?></div>
    ...
    <div class="i_form_buttons">
        ...
        <div class="i_ai_generate transition ownTooltip getAiBox" data-type="aiBox" data-label="<?php echo iN_HelpSecure($LANG['generate_ai_content']);?>">
            <div class="i_pb_aiBox">
                <div class="i_ai_emojis_Box"><?php echo html_entity_decode($iN->iN_SelectedMenuIcon('184'));?></div>
            </div>
        </div>
        <?php } ?>
        ...
        <div class="i_pb_emojis transition">
            <div class="i_pb_emojisBox getEmojis" data-type="emojiBox">
                <div class="i_pb_emojis_Box"><?php echo html_entity_decode($iN->iN_SelectedMenuIcon('25'));?></div>
            </div>
        </div>

        <div class="form_who_see transition">
            <div class="whoSeeBox whs">
                <div class="wBox"><?php echo html_entity_decode($activeWhoCanSee);?></div>
            ...
            <div class="i_choose_ws_wrapper">
                ...
                <div class="i_whoseech_menu_item_out wsUpdate transition <?php echo ($userWhoCanSeePost === 1 || $userWhoCanSeePost === '1') ? 'wselected' : ''; ?>" data-id="1" id="wsUpdate1">
                    <?php echo html_entity_decode($iN->iN_SelectedMenuIcon('50'));?> <?php echo iN_HelpSecure($LANG['weveryone']);?>
                </div>

                <div class="i_whoseech_menu_item_out wsUpdate transition <?php echo ($userWhoCanSeePost === 2 || $userWhoCanSeePost === '2') ? 'wselected' : ''; ?>" data-id="2" id="wsUpdate2">
                    <?php echo html_entity_decode($iN->iN_SelectedMenuIcon('15'));?> <?php echo iN_HelpSecure($LANG['wfollowers']);?>
                </div>

                <?php if($feesStatus === '2'){ ?>
                <div class="i_whoseech_menu_item_out wsUpdate transition <?php echo ($userWhoCanSeePost === 3 || $userWhoCanSeePost === '3') ? 'wselected' : ''; ?>" data-id="3" id="wsUpdate3">
                    <?php echo html_entity_decode($iN->iN_SelectedMenuIcon('51'));?> <?php echo iN_HelpSecure($LANG['wsubscribers']);?>
                </div>

                <div class="i_whoseech_menu_item_out wsUpdate transition <?php echo ($userWhoCanSeePost === 4 || $userWhoCanSeePost === '4') ? 'wselected' : ''; ?>" data-id="4" id="wsUpdate4">
